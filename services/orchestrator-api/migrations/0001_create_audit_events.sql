...
-- Skips if the table already exists in the audit schema (moved by 0002).

DO $$
BEGIN
  -- Skip if already migrated to audit schema
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'audit' AND table_name = 'audit_events'
  ) THEN
    RAISE NOTICE '0001: audit.audit_events exists, skipping public creation';
    RETURN;
  END IF;

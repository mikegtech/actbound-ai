...
import sitemap, { type MenuItem, type SubMenuItem } from "routes/sitemap";
...
const StackedSidenav = () => {
    ...
    (item: MenuItem) => {
      const checkLink = (subMenuItem: SubMenuItem) => {
        if (
          ...
          (subMenuItem.selectionPrefix &&
            pathname?.includes(subMenuItem.selectionPrefix))
        ) {
          return true;
        }
        return subMenuItem.items?.some(checkLink);
      };
  ...
  // Map Auth0 user safely
  const user = useMemo(() => {
    return auth0User
      ? { name: auth0User.name || auth0User.email, avatar: auth0User.picture }
      : { name: "Guest", avatar: undefined };
  }, [auth0User]);
      ...
      sx={[
        {
          ...
          position: {
            md: "absolute",
            lg: mouseEntered ? "absolute" : "static",
          },
